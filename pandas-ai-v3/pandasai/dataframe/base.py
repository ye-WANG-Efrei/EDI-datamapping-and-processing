...
class DataFrame(pd.DataFrame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if _table_name:
            ...
            #print(f"[DEBUG DataFrame.__init__] self._table_name set to: {self._table_name}")
    ...
    def get_default_schema(cls, dataframe: DataFrame) -> SemanticLayerSchema:
        ...
        base_table_name = getattr(
            dataframe, "_table_name", f"table_{dataframe._column_hash}"
        )
        #print(f"[DEBUG get_default_schema] base_table_name from getattr: {base_table_name}")

        final_sanitized_name = sanitize_sql_table_name(str(base_table_name))
        #print(f"[DEBUG get_default_schema] final_sanitized_name before SemanticLayerSchema: {final_sanitized_name}")

        return SemanticLayerSchema(
            name=final_sanitized_name,
            source=Source(
                type="parquet",
                path="data.parquet",
            ),
