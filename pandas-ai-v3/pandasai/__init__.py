...
def read_excel(filepath: str) -> Union[DataFrame, List[DataFrame]]:
    ...
    if not data_dic: # No sheets found
        ...
        # Only one sheet, return a single DataFrame with sanitized sheet name
        ...
        for original_sheet_name, pandas_df in data_dic.items():
            ...
            sanitized_sheet_name = sanitize_sql_table_name(str(original_sheet_name))
            # Combine sanitized filename and sheet name
            combined_name = f"{sanitized_base_filename}_{sanitized_sheet_name}"
            
            #print("combined_name: ", combined_name)     
            # Sanitize the combined name again to be safe (length, etc.)
            final_table_name = sanitize_sql_table_name(combined_name)
            # --- DEBUG PRINT --- 
            #print(f"[DEBUG read_excel] final_table_name to be used for _table_name: {final_table_name}")
            # Create PandasAI DataFrame and add to list
            pai_df = DataFrame(pandas_df, _table_name=final_table_name)
            result_list.append(pai_df)
